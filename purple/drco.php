...
<br>
		...
		?>
				<?php
				$SQLGetAssets = $odb -> query("SELECT ID,name,ip,os,team,url FROM `purpleservers` WHERE os LIKE '7'");
				$drcoR = $SQLGetAssets -> fetchAll(PDO::FETCH_ASSOC);
	            if(!empty($drcoR)){
	                foreach ($drcoR as $key => $getInfo) {	
						...
						if ($user -> isAdmin($odb)) {
							...
							echo '<center><a class="btn btn-outline-warning mb-2 me-4" href="../admin/editsys.php?id='.$id.'" role="button">Edit System</a><a class="btn btn-outline-danger mb-2 me-4" href="../purple/drco.php?deleteId='.$id.'" name="deleteBtn" value="'.$id.'" role="button" type="submit">Delete</a></center></td></tr>'; } else { echo '</td></tr>';
						}
					}
				}else{
					echo '<tr class=""><td valign="top" colspan="5" class="dataTables_empty" style="text-align:center;">There are currently no dracos linux systems  in list,</td></tr>';
				}
				?>
                                    </tbody>
                                </table>
