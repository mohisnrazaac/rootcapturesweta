...
$getid = $_GET['id'];
    $SQLCSelect = $odb -> query("SELECT * FROM `knowledge_base` where id = $getid");
    $SQLCSelect -> execute();
    $knowledgeData =  $SQLCSelect -> fetchAll(PDO::FETCH_ASSOC); 
    $knowledgeDatas = null;
    if(isset($knowledgeData[0])){
        $knowledgeDatas = $knowledgeData[0];
        $kcatId = $knowledgeDatas['category_id'];
        $SQLCSelect = $odb -> query("SELECT * FROM `sub_category` where category_id = $kcatId");
        $SQLCSelect -> execute();
        $subcat =  $SQLCSelect -> fetchAll(PDO::FETCH_ASSOC); 
        // print_r($knowledgeData); exit;
    }else{
...
// print_r($_POST); exit;	
	...
	{   
        try
        {          
            $SQLupdate = $odb -> prepare("UPDATE knowledge_base SET `category_id` = :category_id, sub_category_id = :sub_category_id, content = :content WHERE id = :id");
				$SQLupdate -> execute(array(':category_id' => $category_id, ':sub_category_id' => $sub_category, ':content' => $detailAdd, ':id' => $getid));    
...
<?php include '../sidebar.php'; ?>
		...
		{
			if(empty($errors)) {
				echo '<div class="message" id="message"><p><strong>SUCCESS: The Knowledgebase has been updated! You are now being redirected to the Listing.</strong></div><meta http-equiv="refresh" content="2;url=../admin/knowledgebase.php">';
        ...
        if($knowledgeDatas == null){
            echo '<div class="error" id="message"><p><strong> Data not found.</strong></div><meta http-equiv="refresh" content="1;url=../admin/knowledgebase.php">';
        }
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 mt-md-0 mt-4">
                                            ...
                                            <select name="category" id="category" class="form-control mb-3">
                                                ...
                                                <?php
                                                    foreach($cat  as $key=>$value){
                                                        if(isset($knowledgeDatas['category_id']) && $value['enterprise_id'] == $knowledgeDatas['category_id']){
                                            ...
                                            <select name="sub_category" id="sub_category" class="form-control mb-3">
                                                ...
                                                <?php
                                                    foreach($subcat  as $key=>$value){
                                                        if(isset($knowledgeDatas['sub_category_id']) && $value['enterprise_id'] == $knowledgeDatas['sub_category_id']){
                                                            echo "<option value='".$value['enterprise_id']."' selected>".$value['name']."</option>";
                                                        }else{
                                                            echo "<option value='".$value['enterprise_id']."'>".$value['name']."</option>";
                                                        }
                                    ...
                                    <div class="col-md-12">
                                        <div  class="form-group customTextAreaTxt">
                                            ...
                                            if(isset($knowledgeDatas['content'])){
                                                echo $knowledgeDatas['content'];
                                            }
                                            ?>
                                            </div>
