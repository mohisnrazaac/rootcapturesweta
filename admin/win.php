...
if ($user -> isPurpleTeam($odb) || $user -> isAssist($odb) || $user -> isAdmin($odb)) {
...
<br>
                ...
                <th class="text-center" scope="col">System IP</th>
				<th class="text-center" scope="col">Operating System</th>
				<th class="text-center" scope="col">System Team Assignment</th>
				<th class="text-center" scope="col">System Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
										<?php
		if (isset($_GET['deleteId']))
		{
			...
			//{
				$SQL = $odb -> prepare("DELETE FROM `purpleservers` WHERE `ID` = :id LIMIT 1");
		...
		?>
				<?php
				$SQLGetAssets = $odb -> query("SELECT ID,name,ip,os,team,url FROM `purpleservers` WHERE os LIKE '1'");
				$winR = $SQLGetAssets -> fetchAll(PDO::FETCH_ASSOC);
                if(!empty($winR)){
                    foreach ($winR as $key => $getInfo) {
                        ...
                        if ($user -> isAdmin($odb)) {
                            ...
                            echo '<center><a class="btn btn-outline-warning mb-2 me-4" href="../admin/editsys.php?id='.$id.'" role="button">Edit System</a><a class="btn btn-outline-danger mb-2 me-4" href="../purple/win.php?deleteId='.$id.'" name="deleteBtn" value="'.$id.'" role="button" type="submit">Delete</a></center></td></tr>'; } else { echo '</td></tr>';
                        }
                    }
                }else{
                    echo '<tr class=""><td valign="top" colspan="5" class="dataTables_empty" style="text-align:center;">There are currently no window system in list,</td></tr>';
                }
                ?>
                                    </tbody>
                                </table>
