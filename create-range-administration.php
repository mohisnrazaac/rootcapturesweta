...
require_once '../includes/init.php';
    ...
    $SQLGetTeam = $odb -> query("SELECT * FROM `teams` WHERE name != 'Admin' AND name != 'Administrative Assistant'")->fetchAll();
    $SQLGetallTeam = $user -> getTeamList($odb);
    ...
    $getActiveInactiveSession = $user -> getActiveInactiveSession($odb); 
    ...
    if (isset($_POST['resetCyberRange']))
    {
        ...
        $SQL -> execute(array(':username' => 'AdminDemo')); // remove all user except the AdminDemo

        $SQL = $odb -> query("TRUNCATE `news`"); // remove announcement
        //remove all grade
        $SQL = $odb -> query("TRUNCATE `grading_rubric_criteria`");
        $SQL = $odb -> query("TRUNCATE `grading_rubric_criteria_user`");

        $SQL = $odb -> query("TRUNCATE `asset`"); // remove all asset
        $SQL = $odb -> query("TRUNCATE `asset_group`"); // remove all group

        $SQL = $odb -> query("TRUNCATE `tickets`"); // remove all tickets
        $SQL = $odb -> query("TRUNCATE `ticketResponses`"); // remove all tickets
    ...
    if (isset($_POST['resetAssetsGroupList']))
    {
        ...
        $SQL = $odb -> query("TRUNCATE `asset_group`");
    ...
    {
        $SQL = $odb -> query("TRUNCATE `recent_activities`");
    ...
    //Queries to get total assets
        ...
        $sqlTotalAsset = $sqlTotalAsset->fetchColumn();
                                ...
                                <div class="widget widget-activity-four">
                                   ...
                                   <div class="tab-pane fade" id="animated-underline-range-setting" role="tabpanel" aria-labelledby="animated-underline-range-setting-tab">    
                                    ...
                                    <?php echo 'test'; exit;
                                      ...
                                      echo  $isRegistrationMode = $user -> isRegistrationMode($odb,$college_id);  exit;
                                        $isRedTeamCTFactive = $user -> isRedTeamCTFactive($odb);                                         
                                    ?>
