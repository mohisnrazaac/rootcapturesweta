    ...
    $.ajax({
        ...
        success:function(response){
            ...
            channel.bind('send_message',function(data){ 
                if(data.rec_id == loggedUserId){

                    var text = `<li class="chat_list" chat="${data.message}">
                        <div class="conversation-list">
                            <div class="avatar-xs">
                                <span class="avatar-title rounded-circle bg-soft-primary text-primary">${data.rec_name[0]}</span>
                            </div>
                                ...
                                <div class="ctext-wrap">
                                    ...
                                    <div class="dropdown align-self-start">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item" href="javascript:void(0);" onclick='copyMessage("${data.message}")'>Copy <i class="ri-file-copy-line float-end text-muted"></i></a>
                                            <a class="dropdown-item" href="javascript:void(0);" onclick='forwardMessage("${data.message}")'>Forward <i class="ri-chat-forward-line float-end text-muted"></i></a>
                                        </div>
                                    </div>
                                </div>
                                <div class="conversation-name m-2">${data.rec_name}</div>
                            </div>
                        </div>
                    </li>`;
        
                    $('.websocket-chat-box').append(text);
                   
                    $('.websocket-chat-box').animate({
                        scrollTop: $('.websocket-chat-box .chat_list:last-child').position().top
                    }, 'slow');
                }else{
                    console.log('aaa');
                        var text = `<li class="right chat_list" chat="${data.message}">
                        <div class="conversation-list">
                            ...
                            <div class="user-chat-content">
                                <div class="ctext-wrap">
                                    <div class="ctext-wrap-content">
                                        ...
                                        <p class="chat-time mb-0"><i class="ri-time-line align-middle"></i> <span class="align-middle">Just Now</span></p>                               
                        ...
                        $('.websocket-chat-box').animate({
                            scrollTop: $('.websocket-chat-box .right:last-child').position().top
                        }, 'slow');
                }
...
function openGroupChat(group_id,loggedUserId){
    $.ajax({
        type:"post",
        url:"chat-ajax.php",
        ...
        success:function(response){
            ...
            var channel = pusher.subscribe(res[0]);
            channel.bind('group_chat',function(data){   console.log(data);
                if(data.loggedUserId == loggedUserId){
                    var text = `<li class="right chat_list" chat="${data.message}">
                    <div class="conversation-list">
                        <div class="chat-avatar sender-avatar">
                            <span class="avatar-title rounded-circle bg-soft-primary text-primary">${data.rec_name[0]}</span>
