...
if($token==''){
	...
	$sql -> execute(array(":api_function" => "Delete Grading Rubric Criterion"));
			...
			if(in_array($user_id, $api_members)){ 
				if($rubric_id!=''){
					...
					if($rubric){
                        ...
                        $SQL1 = $odb -> prepare("DELETE FROM `grading_rubric_criteria_user` WHERE `grading_rubric_criteria_id` = :id");
                        $SQL1 -> execute(array(':id' => $rubric_id));

						$response['type'] = "success";
						$response['msg'] = "Grading Rubric Criterion delete successfully.";
