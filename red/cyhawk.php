...
if ($user -> isRedTeam($odb) || $user -> isAssist($odb) || $user -> isAdmin($odb)) {
...
<br>
		...
		?>
				<?php
				$SQLGetAssets = $odb -> query("SELECT ID,name,ip,os,team,url FROM `redservers` WHERE os LIKE '10'");

				$cyhawkR = $SQLGetAssets -> fetchAll(PDO::FETCH_ASSOC);
	            if(!empty($cyhawkR)){
	                foreach ($cyhawkR as $key => $getInfo) {	
						...
						echo '<center><a class="btn btn-outline-warning mb-2 me-4" href="../admin/editsys.php?id='.$id.'" role="button">Edit System</a><a class="btn btn-outline-danger mb-2 me-4" href="../red/cyhawk.php?deleteId='.$id.'" name="deleteBtn" value="'.$id.'" role="button" type="submit">Delete</a></center></td></tr>'; } else { echo '</td></tr>';
						}
					}
				}else{
					echo '<tr class=""><td valign="top" colspan="5" class="dataTables_empty" style="text-align:center;">There are currently no cyborg hawk system in list,</td></tr>';
				}
				?>
                                    </tbody>
                                </table>
