...
<br>
		...
		?>
				<?php
				$SQLGetAssets = $odb -> query("SELECT ID,name,ip,os,team,url FROM `redservers` WHERE os LIKE '2'");
				$kaliR = $SQLGetAssets -> fetchAll(PDO::FETCH_ASSOC);
	            if(!empty($kaliR)){
	                foreach ($kaliR as $key => $getInfo) {						
					...
					echo '<tr class=""><td valign="top" colspan="5" class="dataTables_empty" style="text-align:center;">There are currently no kali linux system in list,</td></tr>';
				}
				?>
                                    </tbody>
                                </table>
