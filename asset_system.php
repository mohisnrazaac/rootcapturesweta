...
<br>
                    ...
                    <div class="page-meta">
                        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><?php echo $group.' asset list'; ?></li>
                            </ol>
                        </nav>
                    </div>
                    <!-- /BREADCRUMB -->
		...
		if (isset($_GET['deleteId']))
		{
			...
			// 	$SQL = $odb -> prepare("DELETE FROM `redservers` WHERE `ID` = :id LIMIT 1");
			// 	$SQL -> execute(array(':id' => $delete));
			
			// echo '<div class="message" id="message"><p><strong>SUCCESS: </strong>The asset has been deleted!</p></div>';
		}

                $SQL = $odb -> prepare("SELECT * FROM `asset_group` WHERE `name` LIKE :name");
                $SQL -> execute(array(':name' => $group)); 
                if($SQL->rowCount()>0)
                {
                    $groupD = $SQL -> fetch(); 
                    $groupId = $groupD['id'];
    
                    $SQLGetAssets = $odb -> query("SELECT * FROM `asset` WHERE `asset_group` = $groupId");
                    
                    while($getInfo = $SQLGetAssets ->fetch())
                    {
                        
                        $name = htmlspecialchars($getInfo['system_name']);
                        $ip = $getInfo['system_ip'];
                        $os = $getInfo['operating_system'];
                        $teamassignment =  $getInfo['team'];
                        $url = $getInfo['url'];
                        $id = $getInfo['id'];
                        
                        $SQL = $odb -> prepare("SELECT name,color_code FROM `teams` WHERE `id` = :id");
                        $SQL -> execute(array(':id' => $teamassignment));
                        $data = $SQL -> fetchAll();	
                        $color_code = $data[0]['color_code'];
                        $team_name = $data[0]['name'];
                        
                        $teamassignment = '<span class="team-btn" style="background-color: '.$color_code.';">'.$team_name.'</span>';
                        ...
                        echo '<tr><td>' . $name  . '</td><td><center>' . $ip . '</center></td><td><center>' . $os .'</center></td><td><center>' . $teamassignment . '</center></td><td><center><a class="btn btn-outline-success mb-2 me-4" href="'.$url.'" role="button">Enter System</a></center>'; if ($user -> isAdmin($odb) || $user -> isAssist($odb)) {
    
                        echo '<center><a class="btn btn-outline-warning mb-2 me-4" href="" role="button">Edit System</a><a class="btn btn-outline-danger mb-2 me-4" href="" name="deleteBtn" value="'.$id.'" role="button" type="submit">Delete</a></center></td></tr>'; } else { echo '</td></tr>';
                        // echo '<center><a class="btn btn-outline-warning mb-2 me-4" href="./admin/editsys.php?id='.$id.'" role="button">Edit System</a><a class="btn btn-outline-danger mb-2 me-4" href="./red/kali.php?deleteId='.$id.'" name="deleteBtn" value="'.$id.'" role="button" type="submit">Delete</a></center></td></tr>'; } else { echo '</td></tr>';
                        }
                    }
                }
